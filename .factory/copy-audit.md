# Landing copy audit

Audit date: 2026-08-29, polish round 3. Counts treat hyphenated terms and
prices as one word. The inventory covers the landing route, demo first screen,
and the empty, imported, mapping, and paid states. No sentence exceeds 22
words. No banned word appears.

| Copy | Words | Result |
|---|---:|---|
| Completed work to review before invoicing | 6 | Pass |
| Find finished work you have not billed | 7 | Pass |
| For freelancers and tiny agencies with work spread across task, time, and invoice tools. | 14 | Pass |
| Try it with sample data | 6 | Pass |
| See a filled review in one click. | 7 | Pass |
| Or import your CSV files | 5 | Pass |
| Files stay in this browser. | 5 | Pass |
| Works offline after your first visit. | 6 | Pass |
| Imports and checklist exports are free. Review history costs $19 once. | 11 | Pass — `free-core`, `billing-boundary` claims |
| Review completed work that may still need an invoice. | 8 | Pass |
| Your local workspace | 3 | Pass |
| Import work and invoice CSV files | 6 | Pass |
| Add one completed-work export from each tool. | 7 | Pass — `multi-source-import` claim |
| You choose whether a suggested invoice match counts as billed. | 9 | Pass |
| Completed work CSVs | 3 | Pass |
| Add one export from each task or time tool. | 9 | Pass — `multi-source-import` claim |
| Invoices CSV | 2 | Pass |
| Issued or draft invoices with client and project names. | 9 | Pass |
| Add another export | 3 | Pass — `multi-source-import` claim |
| Replace this source | 3 | Pass — `multi-source-import`, `work-replacement` claims |
| Map completed work columns | 4 | Pass |
| Map invoice columns | 3 | Pass |
| Required fields are marked. | 4 | Pass |
| Dates must use YYYY-MM-DD or M/D/YYYY. | 6 | Pass |
| This will replace only [source] after you confirm. | 8 | Pass — `multi-source-import` claim |
| hours multiplied by rate must produce a finite amount | 9 | Pass |
| 1 prior review decision was cleared because that work changed. | 11 | Pass |
| Your unbilled-work list will appear here | 6 | Pass |
| Import completed work first. | 4 | Pass |
| Add invoices to review possible matches. | 6 | Pass |
| Already billed and unfinished rows are excluded. | 7 | Pass — `queue-filter` claim |
| No completed work needs review | 5 | Pass |
| Import more work, or unlink a match to bring it back. | 11 | Pass |
| Link an invoice or keep the completed work in your unbilled-work list. | 12 | Pass |
| Download the reviewed list as a CSV for your invoicing session. | 11 | Pass |
| What this tool does not do | 6 | Pass |
| It does not send invoices | 5 | Pass |
| It does not track time, calculate tax, or change your source files. | 12 | Pass |
| It only helps you review exported records. | 7 | Pass |
| Saved review history | 3 | Pass |
| Save weekly review totals for a one-time $19 | 8 | Pass |
| Name each weekly review and compare earlier list totals on this device. | 11 | Pass — `snapshot-history` claim |
| Imports and checklist exports stay free. | 7 | Pass |
| Buy review history — $19 | 4 | Pass |
| View review history options | 4 | Pass |
| Checkout opens through Sociobot. One payment; no subscription. | 8 | Pass |
| Have a license? Paste it here | 6 | Pass |
| Buying means you accept the terms and privacy notice. | 9 | Pass |
| Find completed work that still needs an invoice. | 8 | Pass |
| Artwork disclosure: generated for this product. | 6 | Pass |

## Terminology table

| Concept | One term used |
|---|---|
| The current list of possibly unbilled work | unbilled-work list |
| A source row describing finished work | completed work |
| A candidate relationship to an invoice | suggested match |
| A confirmed work-to-invoice relationship | linked match |
| The downloadable billing preparation file | checklist |
| Saved paid list totals | review history |
| The isolated sample workspace | demo |
| The user’s current real data | workspace |
| One imported task or time CSV | source |

## Demo first screen

| Copy | Words | Result |
|---|---:|---|
| Demo — sample data, nothing is saved | 7 | Pass — `demo-sample-ready` claim |
| Link or keep a suggested match, then export the list. | 10 | Pass |
| Reset demo | 2 | Pass |
| Start for real | 3 | Pass |
| Ready-to-review sample | 2 | Pass |
| Review work before you invoice | 5 | Pass |
| Six work rows, two suggested matches, and four items to review. | 10 | Pass — `demo-sample-ready` claim |
| View review history options | 4 | Pass |
